...
class Relatorios extends My_Controller {
	...
	public function consultar_resultados(){
		...
		if($this->model_users->ver_acesso($this->uri->segment(2))) {
			...
			if($this->session->userdata('user_group')==1 || $this->session->userdata('user_group')==2 || $this->session->userdata('user_group')==4){//verifica se o usuário é do tipo administrador ou consultor
	...
	public function regiao(){
		if($this->model_users->ver_acesso('filtro_regiao')) {
			if($this->session->userdata('user_group')==1 || $this->session->userdata('user_group')==2 || $this->session->userdata('user_group')==4){
				...
				$usuario = $this->model_users->get_ByNome($nome_usuario);
				$ubs_regiao = $this->model_ubs->retorna_ubs_byregiao($usuario->id_regiao);
				
				 $dados = array (
			            'titulo' => 'Resultados por Região',
			            'tela' => 'relatorios/consultar_regiao',
			            'ubs_regiao' => $ubs_regiao
			        );
	...
	public function retorno_filtro(){
		if($this->model_users->ver_acesso('filtro_regiao')) {
			$regiao = $this->model_ubs->retorna_ubs_byregiao($this->input->post('regionalsaude'))->result();
